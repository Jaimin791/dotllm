...
describe('Framework Detection', () => {
  ...
  it('should detect Laravel from dependencies', async () => {
    const { detectFrameworks } = await import('../src/analyzers/framework.js');

    const deps = [{ name: 'laravel/framework', version: '^10.0', isDev: false }];

    const frameworks = detectFrameworks(deps);
    expect(frameworks).toContain('laravel');
  });

  it('should detect Lumen as Laravel', async () => {
    const { detectFrameworks } = await import('../src/analyzers/framework.js');

    const deps = [{ name: 'laravel/lumen', version: '^8.0', isDev: false }];
...
describe('AI Framework Detection', () => {
  ...
  it('should detect LangGraph from dependencies', async () => {
    ...
    const deps = [{ name: 'langgraph', version: '0.1.0', isDev: false }];
  ...
  it('should detect LangSmith from dependencies', async () => {
    ...
    const deps = [{ name: 'langsmith', version: '0.1.0', isDev: false }];
  ...
  it('should detect CrewAI from dependencies', async () => {
    ...
    const deps = [{ name: 'crewai', version: '0.1.0', isDev: false }];
  ...
  it('should detect AutoGen from dependencies', async () => {
    ...
    const deps = [{ name: 'pyautogen', version: '0.2.0', isDev: false }];
  ...
  it('should detect LlamaIndex from dependencies', async () => {
    ...
    const deps = [{ name: 'llama-index', version: '0.10.0', isDev: false }];
  ...
  it('should detect OpenAI from dependencies', async () => {
    ...
    const deps = [{ name: 'openai', version: '4.0.0', isDev: false }];
  ...
  it('should detect Anthropic from dependencies', async () => {
    ...
    const deps = [{ name: '@anthropic-ai/sdk', version: '0.20.0', isDev: false }];
  ...
  it('should add langchain if only langchain-core is detected', async () => {
    ...
    const deps = [{ name: 'langchain-core', version: '0.1.0', isDev: false }];
...
describe('Rule Generation', () => {
  ...
  it('should generate PHP rules for PHP projects', async () => {
    const { getApplicableRules } = await import('../src/rules/index.js');
    ...
    const rules = getApplicableRules(mockAnalysis);
    expect(rules.some((r) => r.id === 'php')).toBe(true);
  });

  it('should generate Laravel rules for Laravel projects', async () => {
    const { getApplicableRules } = await import('../src/rules/index.js');
    ...
    const rules = getApplicableRules(mockAnalysis);
    expect(rules.some((r) => r.id === 'laravel')).toBe(true);
  });

  it('should include both PHP and Laravel rules for Laravel projects', async () => {
    const { getApplicableRules } = await import('../src/rules/index.js');
...
describe('Context Markdown Generation', () => {
  ...
  it('should include critical paths if detected', async () => {
    ...
    const mockAnalysis = createMockAnalysis({
      criticalPaths: [{ path: 'src/auth', risk: 'HIGH', reason: 'Authentication logic' }],
    });
...
describe('Error Classes', () => {
  ...
  it('should create error with code and context', async () => {
    ...
    const error = new CodebaseContextError('Test error', 'TEST_ERROR', { foo: 'bar' });
  ...
  it('should export specialized error types', async () => {
    const { FileNotFoundError, ParseError, AnalysisError } = await import('../src/errors.js');
